...
public class OrderController {
	...
	public String list(Model model) {
		...
		for (int i = 0; i < purchaselist.size(); i++) {
			total +=(purchaselist.get(i).getBookCnt() * purchaselist.get(i).getPrice());
		}
...
////		// 총합이 shoppoint보다 클때 전체shoppoint 쓰기
	...
	public String update_success_payment(Model model, DeliveryVO vo1) {
		...
		int total = 0;
		for (int i = 0; i < list3.size(); i++) {
			total +=(list3.get(i).getBookCnt() * list3.get(i).getPrice());
		}
...
////		수정요함
	...
	//	vo.setId(id);
		...
		HttpSession session = request.getSession(); // 세션영역을 가져온다
	      session.setAttribute("money", update_money);
